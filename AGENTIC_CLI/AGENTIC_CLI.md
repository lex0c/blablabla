...
| [`NAVIGATION.md`](./NAVIGATION.md) | Estratégia de navegação de repositório em runtime — grep-first, targeted reads, exploration loops, repo map on-demand, sem index pré-computado. ADR registrando reversão do antigo `CODE_INDEX.md` | Ao desenhar tool de busca/leitura; ao tunar repo map; ao avaliar se vale (re)introduzir indexação |
